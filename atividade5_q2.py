...
def identificadorSVM(t, y, x_test):
    yBinSetosa      = np.where(y == especies['setosa'], 1, -1)
    yBinVersicolor  = np.where(y == especies['versicolor'], 1, -1)
    yBinVirginica   = np.where(y == especies['virginica'], 1, -1)
    ...
    funcao = pos(1 - multiply(yBinSetosa, (a.T @ t + b)))
    ...
    funcao      = pos(1 - multiply(yBinVersicolor, (a.T @ t + b)))
    objective   = Minimize(sum(funcao))
    problema    = Problem(objective)
    problema.solve()
    ...
    funcao      = pos(1 - multiply(yBinVirginica, (a.T @ t + b)))
    objective   = Minimize(sum(funcao))
    problema    = Problem(objective)
    problema.solve()
...
#                                  INICIO ID SVM
######################################################################################
y_predTeste, y_predTreinamento = identificadorSVM(xTrain, yTrain, xTest)
...
print('\nMatriz de Confusão SVM -> Variáveis de teste:')
print(matrizConfusaoTeste)
